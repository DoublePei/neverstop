package com.ns.net.common.repository;

import com.ns.net.common.model.bean.JobTaskState;
...
public interface SchedulerTaskRepository extends PagingAndSortingRepository<SchedulerTask, Long> {
    ...
    @Query(rowMapperClass = RunningJobCountMapper.class, value = "select worker,count(*) as cnt from t_kepler_scheduler_task where worker is not null group by worker")
    List<Map<String, Object>> getRunningJobCount();

    @Query("select * from t_kepler_scheduler_task where job_id = :jobId and schedule_time = :scheduleTime order by start_time desc")
    List<SchedulerTask> getInstancesByIdAndTime(Long jobId, String scheduleTime);

    @Query("select * from t_kepler_scheduler_task where worker_host = :ip and task_state in (:states)")
    List<SchedulerTask> findAllTaskByWorkerHostAndTaskStates(String ip, List<Integer> states);

    @Query(rowMapperClass = AllJobStatusMapper.class, value = "select id,job_id,task_state,schedule_time from t_kepler_scheduler_task where schedule_time is not null and schedule_time and schedule_time > :startTime and schedule_time < :endTime")
    List<JobTaskState> getAllJobStatus(String startTime, String endTime);

    @Query("select * from t_kepler_scheduler_task where task_state in (:states) order by schedule_time asc")
    List<SchedulerTask> findTasksByTaskStates(List<Integer> states);

    @Query("select * from t_kepler_scheduler_task where job_id = :jobId and task_state in (:states)")
    List<SchedulerTask> findTasksByTaskStatesAndJobId(Long jobId, List<Integer> states);

    @Query("select * from t_kepler_scheduler_task where dag_id = :dagId order by create_time")
    List<SchedulerTask> findAllByDagId(Long dagId);

    @Modifying
    @Query("update t_kepler_scheduler_task set task_state = :state where id in (:ids)")
    Integer updateJobStatus(List<Long> ids, Integer state);

    @Modifying
    @Query("delete from t_kepler_scheduler_task where job_id = :jobId")
    Integer deleteAllByJobId(Long jobId);

    @Modifying
    @Query("update t_kepler_scheduler_task set task_state = :state where job_id = :id and schedule_time = :scheduleTime")
    Integer updateJobStatusAsFailed(Integer state, Long jobId, String scheduleTime);

    @Query("select * from t_kepler_scheduler_task where job_id = :jobId and schedule_time < :scheduleTime order by schedule_time, start_time")
    List<SchedulerTask> getTaskList(Long jobId, String scheduleTime);

    @Query("select b.* from (select max(id) as id from t_kepler_scheduler_task  where job_id = :id and schedule_time =:time group by schedule_time ,job_id ) a  join  ( select * from t_kepler_scheduler_task where job_id  = :id and schedule_time = :time ) b on a.id = b.id")
    List<SchedulerTask> findTaskByChildTime(Long id, String time);

    @Query("select * from t_kepler_scheduler_task where job_id = :jobId and DATE_FORMAT(schedule_time,'%Y-%m-%d-%H') = :executionDate")
    List<SchedulerTask> findByExecutionDateByHour(Long jobId, String executionDate);

    @Query("select * from t_kepler_scheduler_task where job_id = :jobId and DATE_FORMAT(schedule_time,'%Y-%m-%d') = :executionDate")
